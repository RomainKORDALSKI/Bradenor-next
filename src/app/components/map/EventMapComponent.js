...
const EventMapComponent = ({ events }) => {
  ...
  // Génère les liens Waze et Google Maps
  ...
  return (
    <div>
      ...
      >
        ...
        {/* Vérifie que les coordonnées sont valides avant de créer un marqueur */}
        {filteredEvents
          .filter((event) => event.latitude && event.longitude) // Filtrer les événements avec coordonnées valides
          .map((event) => {
